  ...
  return (
    <div className="inline-block bg-white p-6 rounded-lg shadow-md">
      <div className="flex items-center">
        <div className="text-4xl font-light text-gray-700 mr-4">[</div>
          ...
          {values.map((row, rowIndex) =>
            row.map((value, colIndex) => (
              <input
                ...
                className="w-16 h-12 text-center border-2 border-gray-300 rounded focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 transition-all"
                placeholder="0"
              />
            ))
          )}
        </div>
        <div className="text-4xl font-light text-gray-700 ml-4">]</div>
      </div>
    </div>
  );
};
